...
class NpsService extends ValidateAbstractService
{
    protected $em;
    ...
    public function sendVote(Form $form,Survey $survey){
        if ($survey->getResendDate() != null){
            ...
            $survey->setResendStaffRate($form->getData()->getStaffRate());
            $this->em->persist($survey);
            $this->em->flush($survey);
        }else{
            ...
            $survey->setStaffRate($form->getData()->getStaffRate());
            $this->em->persist($survey);
            $this->em->flush($survey);
        }
        return $survey;
    ...
    public function importSurvey($data,User $user,SurveyType $surveyType){
        ...
        $staff_rate = null;
        $resend_staff_rate = null;

        if ($data->staff_rate != null){
            $staff_rate = $this->em->getRepository('SurveyBundle:SurveyStaffRate')->find((int)$data->staff_rate + 1);
        }
        if ($data->resend_staff_rate != null){
            $resend_staff_rate = $this->em->getRepository('SurveyBundle:SurveyStaffRate')->find((int)$data->resend_staff_rate + 1);
        }
        ...
        if ($staff_rate) {
            $survey->setStaffRate($staff_rate);
        }
        if ($resend_staff_rate) {
            $survey->setResendStaffRate($resend_staff_rate);
        }
